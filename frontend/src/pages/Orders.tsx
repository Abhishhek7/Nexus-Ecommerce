  ...
  return (
    <section>
      ...
      <div className="table">
        ...
        {loading ? (
          ...
          result.data.map((o: any) => (
            <Link className="tr rowlink" to={`/orders/${o.id}`} key={o.id}>
              <span>
                <b>{o.orderNumber}</b>
                <small>{new Date(o.createdAt).toLocaleDateString('en-IN')}</small>
              </span>
              <span>{o.customer?.name}</span>
              <span>{o.items?.reduce((s: number, i: any) => s + i.quantity, 0)}</span>
      ...
      <div className="pagination">
        ...
        <span>
          Page {page} of {result.pagination?.totalPages || 1} · {result.pagination?.total || 0} orders
        </span>
        <button disabled={page >= result.pagination?.totalPages} onClick={() => setPage(page + 1)}>
          Next
        </button>
      </div>
    </section>
